...
import type { ConfirmationSource, VerificationPeriod } from "../../types";
import styles from "./reputation.module.css";

const sources: readonly ConfirmationSource[] = ["direct", "companion", "reference"];
...
export function VerifiedConfirmationDialog({ noteId, open, onClose, onSuccess }: { readonly noteId?: string; readonly open: boolean; readonly onClose: () => void; readonly onSuccess: (rewardedXp: number) => void }) {
  ...
  const [source, setSource] = useState<ConfirmationSource>("direct");
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      const result = await services.reputation.confirmNote({ noteId, source, verificationPeriod: period, comment: comment.trim() || undefined, evidencePath, idempotencyKey: idempotencyKey("confirm") });
  ...
  return <dialog ref={ref} className={styles.actionSheet} aria-labelledby="confirmation-dialog-title" onClose={onClose}>
    ...
    <form className={styles.actionForm} onSubmit={(event) => void submit(event)}>
      <fieldset><legend>{t("confirmation.how")}</legend>{sources.map((value) => <label key={value}><input type="radio" name="confirmation-source" value={value} checked={source === value} onChange={() => setSource(value)} /><span>{t(`confirmation.source.${value}`)}</span><small>{value === "direct" ? t("confirmation.reward") : t("confirmation.noReward")}</small></label>)}</fieldset>
