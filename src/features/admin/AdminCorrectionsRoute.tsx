...
export function AdminCorrectionsRoute() {
  ...
  useEffect(() => {
    ...
    void services.reputation.listCorrections().then((values) => active && setItems(values)).catch(() => active && setError(true)).finally(() => active && setLoading(false));
    return () => { active = false; };
  }, [services, session]);
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.list}>{items.map((item) => <article key={item.id}>
      <div className={styles.meta}><strong>{item.portKey}</strong><span>{item.fieldType} · @{item.submitterAlias || "CrewPort"}</span></div>
      ...
      <p className={styles.consequence}>{t("admin.corrections.consequence")}</p>
      <div className={styles.actions}><button type="button" onClick={() => void review(item, "rejected")}>{t("admin.corrections.reject")}</button><button type="button" onClick={() => void review(item, "accepted")}>{t("admin.corrections.approve")}</button></div>
    </article>)}</div>
  </main>;
}
