...
begin
  ...
  select * into v_feedback from public.note_feedback
    where id = p_feedback_id and deleted_at is null
      and (user_id = auth.uid() or public.current_app_role() = 'admin')
    for update;
  if v_feedback.id is null then raise exception 'feedback_author_or_admin_required' using errcode = '42501'; end if;
  if exists (
...
begin
  ...
  where id = p_feedback_id and deleted_at is null
    and (user_id = auth.uid() or public.current_app_role() = 'admin');
  if not found then raise exception 'feedback_author_or_admin_required' using errcode = '42501'; end if;
end;
$$;

comment on function public.update_note_feedback(uuid, text) is
  'Allows only the feedback author or an Admin to edit feedback.';

comment on function public.delete_note_feedback(uuid) is
  'Allows only the feedback author or an Admin to soft-delete feedback.';
